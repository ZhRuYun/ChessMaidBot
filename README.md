...
| | 综合 AI 设置弹窗（API连接 + 教学触发器配置 + 自定义 AI 人设 + 调试工具日志） | ✅ 已完成 | 统一持久化存储至 `data/settings.json`，重启不丢失 |
| **2. Controller 调度层** | 对弈模式管理器（扮演双方棋手、人机 Stockfish 对弈、女仆 LLM 陪练、网络双人对战） | ✅ 已完成 | 完整实装选边流程（黑白棋自由选）、网络双人对战极简原型 |
...
| | 标准化上下文请求包（`AgentRequest`）与工具库（`AgentTools`） | ✅ 已完成 | 支持开局库、战术库、残局库、引擎状态读取与联网搜索工具 |
