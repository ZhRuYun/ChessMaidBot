...
| | 主题系统（跟随系统、浅色、深色三档位切换） | ✅ 已完成 | 顶部控制栏实时平滑切换配色主题 |
| | 认输（认输）、求和（求和）与 一键导入/导出（PGN/FEN） | ✅ 已完成 | 零卡顿异步归档，支持智能识别 PGN / FEN 导入 |
| | 综合 AI 设置弹窗（API连接 + 教学触发器配置 + 自定义 AI 人设） | ✅ 已完成 | 统一整合在一处，支持 Base URL/Key/模型/思考档位/流式输出及预设模板 |
| **2. Controller 调度层** | 对弈模式管理器（本地双人、人机 Stockfish 对弈、女仆 LLM 陪练） | ✅ 已完成 | 完整实装 Vs Maid LLM 陪练模式与降级保障机制 |
...
| **5. Agent 接口与方法库** | 自定义 LLM 人设 Prompt 注入与精炼 PromptBuilder | ✅ 已完成 | 输出精简短小（150字内），彻底移除 emoji |
| | 标准化上下文请求包（`AgentRequest`）与工具库（`AgentTools`） | ✅ 已完成 | 支持开局库、战术库、残局库与引擎状态读取 |
