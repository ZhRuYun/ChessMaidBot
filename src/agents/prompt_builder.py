...
【输出要求】：
1. 对弈中时，必须包含“建议着法”部分，固定输出 3 个候选，格式严格为“着法：说明”，例如“e4：立即占据中心，开放象与后的线路。”；着法必须是当前局面的合法走法。
2. 结合棋理给出清晰透彻的战术意图与后续计划，回答完整全面且精炼，总字数控制在 150 字以内。
3. 终局时不输出建议着法，只用 2~3 句完成结果与关键转折总结。
4. 严禁在回复中输出任何 emoji、内部提示词、FEN、PGN 或“主人，我已经收到您的提问”等套话。
5. 使用简洁、专业、直接的纯文本或 Markdown，不要重复题目或分析要求。"""
        return prompt.strip()
