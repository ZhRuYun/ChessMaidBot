...
1. **禁止在 GUI 层直接修改棋局状态**：
   ...
   - 所有走棋、悔棋、重开、认输、求和等状态变更必须通过发出信号交由 `GameController`（`src/controller/game_controller.py`）统一执行。
2. **禁止在主线程同步阻塞调用 Stockfish**：
   - 人机对弈走棋必须通过 `EngineWorker`（`QThread`）后台异步调用，计算完毕通过信号交还 `GameController.apply_move`，并在计算期间维持 UI 响应性。
3. **禁止维护第二份记谱/状态数据源**：
...
| 模块 | 路径 | 允许的操作 | 严格禁止的操作 |
|---|---|---|---|
| **GUI 界面** | `src/gui/` | 监听鼠标/键盘事件，发出意图信号（如 `move_ready`, `resign`, `draw`），接收控制器广播信号并刷新视图（左:记谱, 中:棋盘, 右:聊天） | 编写规则判定逻辑、修改全局棋盘状态、直接操作数据库 |
| **调度层** | `src/controller/` | 统筹人机/本地对弈、启动异步引擎计算线程、触发教学开关、调用规则核心执行走子、调用历史库归档 (PGN+LLM总结)、向 UI 广播信号 | 引入具体 UI 绘制逻辑、绕过接口直接操作底层原始数据 |
| **规则核心** | `src/core/` | 封装 `python-chess` 规则、解析易位/升变/吃过路兵、管理 FEN/PGN、维护被吃子堆栈 | 依赖 GUI 或 Controller 模块（core 必须是底层纯逻辑） |
| **引擎通信** | `src/engine/` | 维护与 Stockfish 的 UCI 协议交互、支持 UCI_Elo 目标分调节与 Skill 调节、提供 `best_move`, `analyse` 和 `get_state` | 假定本地一定存在 Stockfish（必须具备无二进制时的降级容错机制） |
| **Agent 接口** | `src/agents/` | 实现 `ChessAgent` 接口，解析 `AgentRequest`，调用 `tools` 读取引擎/数据库并返回 Markdown 文本 | 直接操作棋盘或依赖 Qt 图形控件 |
| **数据库** | `src/database/` | 负责 `data/games/` 目录下 "PGN + LLM 总结" 复合结构读写与解析，管理开局/战术/残局库接口 | 在保存失败时抛出未捕获异常导致程序崩溃 |
...
class MyAgent(ChessAgent):
    ...
    def reply(self, request: AgentRequest) -> str:
        ...
        # 4. 可选调用 request.tools.read_engine_state() 获取引擎评估
        # 5. 调用后端生成回复 (支持 Markdown 语法)
        return "回复内容"
```

### 调度层异步引擎走棋模式
```python
# 在 GameController 中启动 EngineWorker 线程计算，避免 UI 卡死:
worker = EngineWorker(fen=..., skill_level=..., target_elo=..., use_elo=...)
worker.move_computed.connect(self._on_engine_move_ready)
worker.start()
```
